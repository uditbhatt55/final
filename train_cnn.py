...
def build_model(num_classes: int, image_size: int) -> tuple[tf.keras.Model, tf.keras.Model]:
    ...
    model = tf.keras.Model(inputs, outputs, name="sign_language_mobilenetv2")
...
def main() -> None:
    ...
    model, base_model = build_model(len(class_names), args.image_size)
